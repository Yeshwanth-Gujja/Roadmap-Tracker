(() => {
  ...
  function leaves(list, out = []) {
    for (const n of list) { if (!n.children.length) out.push(n);
  ...
  function find(id, list = roadmap) {
    for (const n of list) { if (n.id === id) return n;
      const x = find(id, n.children); if (x) return x }
    return null
  }

  function parentOf(id, list = roadmap, parent = null) {
    for (const n of list) { if (n.id === id) return parent;
      const x = parentOf(id, n.children, n); if (x !== undefined) return x }
    return undefined
  }

  function percent(list) { const x = leaves(list); return x.length ? Math.round(x.filter(n => n.status === 'completed').length / x.length * 100) : 0 }

  function esc(s) { return String(s ?? '').replace(/[&<>"']/g, m => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' } [m])) }

  function statusLabel(s) { return ({ 'not-started': 'Not started', 'in-progress': 'In progress', completed: 'Completed' })[s] || s }

  function config() {
    try { const c = JSON.parse(localStorage.getItem(KEYS.config) || 'null'); return c && c.owner && c.repo && c.branch && c.token ? c : null } catch { return null }
  }

  function saveLocal() { localStorage.setItem(KEYS.data, JSON.stringify(roadmap)) }

  function remember() { undoStack.push(copy(roadmap)); if (undoStack.length > 100) undoStack.shift();
    redoStack = [] }

  function change(fn, githubMessage = 'Update roadmap') { remember();
    fn();
    saveLocal();
    render();
  ...
  function setSync(kind) {
    const b = $('sync');
    b.className = 'pill ' + (kind === 'ok' ? 'ok' : kind === 'busy' ? 'busy' : kind === 'err' ? 'err' : '');
    b.textContent = kind === 'busy' ? 'Saving to GitHub…' : kind === 'ok' ? 'GitHub synced' : kind === 'err' ? 'GitHub error' : config() ? 'GitHub connected' : 'Local only'
  }
  ...
  function render() {
    ...
    let visible = 0;
    const match = n => {
      const text = [n.name, n.description, n.notes, n.targetDate, ...n.resources].join(' ').toLowerCase();
      return (!q || text.includes(q)) && (st === 'all' || n.status === st) && (di === 'all' || n.difficulty === di) && (pr === 'all' || n.priority === pr)
    };
    ...
    const draw = (n, parent, isRoot = false) => {
      ...
      if (n.children.length) {
        ...
        n.children.forEach(c => { const e = draw(c, n, false); if (e) body.appendChild(e) });
        body.insertAdjacentHTML('beforeend', `<div class="addrow"><button class="addchild">+ Add item</button></div>`);
        wrap.querySelector('.toggle').onclick = () => { n._open = !n._open;
          render() };
        wrap.querySelector('.main').onclick = () => { n._open = !n._open;
          render() };
        wrap.querySelector('.edit').onclick = () => openEditor('edit', n, parent);
        wrap.querySelector('.add').onclick = () => openEditor('add', null, n);
        wrap.querySelector('.addchild').onclick = () => openEditor('add', null, n);
        wrap.querySelector('.del').onclick = () => deleteNode(n.id);
        ...
        wrap.querySelector('.check').onchange = e => change(() => n.status = e.target.checked ? 'completed' : 'not-started', e.target.checked ? 'Complete: ' + n.name : 'Reopen: ' + n.name);
        wrap.querySelector('.edit').onclick = () => openEditor('edit', n, parent);
        wrap.querySelector('.del').onclick = () => deleteNode(n.id);
        setupDrag(wrap, n, parent);
      }
    ...
    roadmap.forEach(n => { const e = draw(n, null, true); if (e) $('roadmap').appendChild(e) });
    if (!visible) $('roadmap').innerHTML =
    ...
    setSync(config() ? (syncing ? 'busy' : 'ok') : '');
  }

  function setupDrag(el, n, parent) {
    el.addEventListener('dragstart', e => { drag = { n, parent };
  ...
  function isInside(a, b) { return a.children.some(c => c.id === b.id || isInside(c, b)) }

  function saveAndCommit(msg) { remember();
    saveLocal();
    render();
    githubCommit(msg) }
  ...
  function openEditor(m, n, p) {
    ...
    $('editorTitle').textContent = m === 'edit' ? 'Edit item' : 'Add item';
    $('editId').value = n?.id || '';
    $('parentId').value = p?.id || '';
    $('name').value = n?.name || '';
    ...
    $('leafFields').style.display = m === 'edit' && n && n.children.length ? 'none' : 'block';
  ...
  $('form').onsubmit = e => {
    ...
    change(() => {
      if (mode === 'edit') {
        ...
        const n = { id: uid(), ...data, type: editingParent?.type === 'Section' ? 'Topic' : 'Skill', children: [] };
        if (editingParent) editingParent.children.push(n);
        else { n.type = 'Section';
          roadmap.push(n) }
      }
  ...
  function expandAll() { all(roadmap).forEach(n => { if (n.children.length) n._open = true });
    render() }

  function collapseAll() { all(roadmap).forEach(n => n._open = false);
    render() }
  ...
  async function gh(path, opts = {}) {
    ...
    if (!r.ok) throw Error((await r.text()).slice(0, 400));
    return r.status === 204 ? null : r.json()
  }

  function b64(s) { const bytes = new TextEncoder().encode(s);
    let bin = '';
    bytes.forEach(b => bin += String.fromCharCode(b)); return btoa(bin) }

  function text64(s) { const bin = atob(s.replace(/\n/g, ''));
    const bytes = Uint8Array.from(bin, c => c.charCodeAt(0)); return new TextDecoder().decode(bytes) }

  async function getRemote() {
    const c = config();
    if (!c) return null;
    try { return await gh(`/repos/${encodeURIComponent(c.owner)}/${encodeURIComponent(c.repo)}/contents/roadmap.json?ref=${encodeURIComponent(c.branch)}`) } catch (e) {
      if (String(e.message).startsWith('{') || !String(e.message).includes('404')) throw e;
      return null
    }
  }
  ...
  async function githubCommit(message = 'Update roadmap') {
    ...
    try {
      ...
      const body = {
        ...
        content: b64(JSON.stringify(roadmap, null, 2) + '\n'),
        branch: c.branch
      };
      if (file?.sha) body.sha = file.sha;
      await gh(`/repos/${encodeURIComponent(c.owner)}/${encodeURIComponent(c.repo)}/contents/roadmap.json`, { method: 'PUT', body: JSON.stringify(body) });
      localStorage.setItem('fsai-last-sync', new Date().toISOString());
      setSync('ok')
    } catch (e) { setSync('err');
      toast('GitHub save failed: ' + e.message) } finally { syncing = false }
  }

  async function pull(show = true) {
    if (!config()) { toast('Connect GitHub first'); return }
    syncing = true;
    setSync('busy');
    try {
      const f = await getRemote();
      if (!f) throw Error('roadmap.json was not found in this repository');
      const remote = JSON.parse(text64(f.content));
      if (!Array.isArray(remote)) throw Error('roadmap.json must contain an array');
      if (show) { remember(); }
      ...
      toast('Pull failed: ' + e.message) } finally { syncing = false }
  }
  ...
  async function connect() {
    ...
    if (!c.owner || !c.repo || !c.token) { toast('Owner, repository and token are required'); return }
    try {
      ...
      close('githubModal');
      await pull(false);
      await githubCommit('Connect / save current state');
      toast('GitHub connected')
    } catch (e) { toast('GitHub connection failed: ' + e.message);
      setSync('err') }
  }
  ...
  async function versions() {
    ...
    try {
      const c = config(),
        xs = await gh(`/repos/${encodeURIComponent(c.owner)}/${encodeURIComponent(c.repo)}/commits?path=roadmap.json&sha=${encodeURIComponent(c.branch)}&per_page=50`);
      $('historyStatus').textContent = `${xs.length} versions found`;
      for (const x of xs) {
        ...
        d.querySelector('button').onclick = () => restore(x.sha);
        $('historyList').appendChild(d)
      }
    } catch (e) { $('historyStatus').textContent = 'Could not load history: ' + e.message }
  }

  async function restore(sha) {
    if (!confirm('Restore this version? It will be committed as a new version.')) return;
    try {
      const c = config(),
        f = await gh(`/repos/${encodeURIComponent(c.owner)}/${encodeURIComponent(c.repo)}/contents/roadmap.json?ref=${encodeURIComponent(sha)}`),
        remote = JSON.parse(text64(f.content));
      remember();
      roadmap = normalize(remote);
      saveLocal();
      render();
      await githubCommit('Restore version ' + sha.slice(0, 7));
      close('historyModal');
      toast('Restored and committed as a new version')
    } catch (e) { toast('Restore failed: ' + e.message) }
  }

  function exportData() {
    const a = document.createElement('a');
    a.href = URL.createObjectURL(new Blob([JSON.stringify(roadmap, null, 2)], { type: 'application/json' }));
  ...
  $('file').onchange = e => {
    ...
    r.onload = () => {
      try {
        ...
        change(() => roadmap = normalize(x), 'Import roadmap');
        toast('Imported')
  ...
  $('addRoot').onclick = () => openEditor('add', null, null);
  $('github').onclick = () => {
    const c = config();
    if (c) { $('owner').value = c.owner;
      $('repo').value = c.repo;
  ...
  $('pull').onclick = () => pull(true);
  $('disconnect').onclick = () => { localStorage.removeItem(KEYS.config);
    setSync('');
    close('githubModal');
  ...
  document.addEventListener('keydown', e => {
    if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'z') { e.preventDefault();
      e.shiftKey ? redo() : undo() } else if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'y') { e.preventDefault();
      redo() } else if (e.key === 'Escape') { ['editor', 'githubModal', 'historyModal'].forEach(close) }
  });

  async function boot() {
    document.body.classList.toggle('dark', localStorage.getItem(KEYS.theme) === 'dark');
    $('theme').textContent = document.body.classList.contains('dark') ? 'Light' : 'Dark';
    let initial = null;
    try {
      const r = await fetch('./roadmap.json', { cache: 'no-store' });
      if (r.ok) initial = await r.json()
    } catch {}
    if (!Array.isArray(initial)) initial = copy(window.ORIGINAL_ROADMAP);
    const local = (() => { try { return JSON.parse(localStorage.getItem(KEYS.data) || 'null') } catch { return null } })();
    roadmap = normalize(local && Array.isArray(local) ? local : initial);
    render();
