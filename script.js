(() => {
    ...
    const draw = (n, parent, isRoot = false) => {
      ...
      if (n.type === 'Section' || n.children.length > 0) {
        ...
        wrap.querySelector('.add').onclick = () =>
          openEditor('add-section', null, n);

        const addChildButton = body.lastElementChild.querySelector('.addchild');
  ...
  function openEditor(m, n, p) {
    ...
    $('editorTitle').textContent =
      m === 'edit' ? 'Edit item' : m === 'add-section' ? 'Add Section' : 'Add item';
    ...
    m === 'add-section' || (m === 'edit' && n && n.children.length) ? 'none' : 'block';
    ...
    change(() => {
      if (mode === 'edit') {
        ...
        const n = {
          id: uid(),
          ...data,
          type: mode === 'add-section' ? 'Section' : editingParent?.type === 'Section' ? 'Topic' : 'Skill',
          children: []
        };
  ...
  function expandAll() {
    all(roadmap).forEach(n => {
      if (n.type === 'Section' || n.children.length > 0) n._open = true
    });
    render()
  }
